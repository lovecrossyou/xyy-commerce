...
class WechatPay {
  ...
  getPrepayId(obj) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      request.post({ url, body: JSON.stringify(that.getUnifiedorderXmlParams(UnifiedorderParams)) }, function(error, response, body) {
        if (!error && response.statusCode === 200) {
          ...
          xml2jsparseString(body, { async: true }, function(error, result) {
            if (error) {
              ...
              const prepay_id = result.xml.prepay_id[0]; // 小程序支付返回这个
              console.log(result);
              // const code_url = result.xml.code_url[0];//PC 扫码支付
              // resolve(code_url);
              resolve(prepay_id);
            }
          });
  ...
  getSign(signParams) {
    ...
    const string = queryString.stringify(newArgs) + '&key=' + this.config.wxpaykey;

    // 生成签名
    return crypto.createHash('md5').update(queryString.unescape(string), 'utf8').digest('hex')
      .toUpperCase();
  }
  ...
  /**
     ...
     * @return {{appId: string, timeStamp: Number, nonceStr: *, package: string, signType: string, paySign: *}}
     */
  ...
  getAccessToken(code, cb) {
    const getAccessTokenUrl = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' + this.config.wxappid + '&secret=' + this.config.wxappsecret + '&code=' + code + '&grant_type=authorization_code';
    request.post({ url: getAccessTokenUrl }, function(error, response, body) {
      console.log('getAccessTokenUrl body', body);
      if (!error && response.statusCode === 200) {
  ...
  code2Session(code, cb) {
    const getSessionnUrl = 'https://api.weixin.qq.com/sns/jscode2session?appid=' + this.config.wxappid + '&secret=' + this.config.wxappsecret + '&js_code=' + code + '&grant_type=authorization_code';
    request.get({ url: getSessionnUrl }, function(error, response, body) {
      console.log('code2Session ', response.statusCode);
      console.log('code2Session body', body);
